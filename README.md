...
Web Push (PWA) notifications for osTicket staff panel. Mirrors email alert logic and sends browser push notifications for ticket events.
...
- **Admin controls** with master switch, per-event toggles, and VAPID key management
...
- osTicket **1.18+**
- PHP **8.0+** with `openssl` extension (for VAPID key generation)
...
Admin global toggle ON?
  > Admin event toggle ON?
    > osTicket alert setting ON?
...
| `class.WebPush.php` | Pure PHP Web Push sender (no Composer dependencies) |
| `assets/push-notifications.js` | Client-side bell/gear UI, preferences modal, service worker registration |
| `assets/push-notifications.css` | Styles for nav icons, modal, toggles, toast, dark mode |
